...
import com.template.states.JointVentureState;
...
// * Contract *
// ************
public class JointVentureContract implements Contract {
    // This is used to identify our contract when building a transaction.
    public static final String ID = "com.template.contracts.JointVentureContract";

    // Create command used to create the contract
    public interface Commands extends CommandData {
        class Create extends TypeOnlyCommandData implements Commands {}
        class Dissolve extends TypeOnlyCommandData implements Commands {}
        class AddMember extends TypeOnlyCommandData implements Commands {}
    }
    ...
    public void verify(LedgerTransaction tx) throws IllegalArgumentException {
        ...
        // ADD RECITALS TO JCT
        if (command instanceof Commands.AddMember) {
            verifyRecitals(tx);
        }
    }
    ...
    private void verifyRecitals(LedgerTransaction tx) {
        final CommandWithParties<Commands.AddMember> addMember = requireSingleCommand(tx.getCommands(), Commands.AddMember.class);
        ...
        requireThat(require -> {
            ...
            // State-specific requirements
            final JointVentureState input = tx.inputsOfType(JointVentureState.class).get(0); // Must only be one in the list
            final List<Party> prevParticipants = input.getEmployer();
            prevParticipants.addAll(input.getContractor());

            final JointVentureState output = tx.outputsOfType(JointVentureState.class).get(0);
