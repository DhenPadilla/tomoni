    ...
    public void confirmAmendDateShouldWork() {
        ...
        ledger(ledgerServices, l -> {
            l.transaction(tx -> {
                ...
                tx.command(requiredSigners, new ScheduleClauseContract.Commands.AcceptExpectedDateModification(0));
                return tx.verifies();
            });
    ...
    public void allEmployersMustBeIncludedInTransaction() {
        ...
        ledger(ledgerServices, l -> {
            l.transaction(tx -> {
                ...
                tx.output(com.template.contracts.ScheduleClauseContract.ID, outputState);
                tx.output(ReportContract.ID, outputReportState);
                tx.command(employerKeys, new ScheduleClauseContract.Commands.AcceptExpectedDateModification(0));
