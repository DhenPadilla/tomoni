 ...
 */
abstract class JCTBaseFlow extends FlowLogic<SignedTransaction> {
    ...
    StateAndRef<JCTState> getJCTStateByLinearId(UniqueIdentifier linearId) throws FlowException {
        ...
        List<StateAndRef<JCTState>> obligations = getServiceHub().getVaultService().queryBy(JCTState.class, queryCriteria).getStates();
        if (obligations.size() != 1) {
            throw new FlowException(String.format("JCT with id %s not found.", linearId));
        }
